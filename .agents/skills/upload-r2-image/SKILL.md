...
description: 로컬 파일 또는 웹(http/https) 이미지를 Cloudflare R2에 단건·다건 업로드하고 공개 URL을 반환한다. Cloudflare REST API를 사용하며 AWS SDK는 쓰지 않는다. 사용자가 이미지를 R2 경로에 올려 달라고 하거나, 업로드 결과 URL을 요청할 때 사용한다. 자격증명은 Codex Cloud Secrets에서 읽는다.
---
...
프로젝트 `.env`에 의존하지 않는다. 자격증명은 **Codex Cloud Secrets**에 등록된 환경변수만 사용한다.

## Codex Cloud Secrets 설정
...
| API Token  | (필수)                           | `CLOUDFLARE_API_TOKEN`                       |
| 공개 URL   | `https://images.zzz.freevue.dev` | `R2_PUBLIC_URL`                              |

인증은 **Cloudflare API Token (Bearer)** 만 사용한다. `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY`(S3 호환)는 사용하지 않는다.
...
2. `CLOUDFLARE_API_TOKEN`, `R2_ACCOUNT_ID`(또는 `CLOUDFLARE_ACCOUNT_ID`)가 `process.env`에 있는지 확인한다. 없으면 Codex Cloud Secrets 등록을 안내하고 중단한다.
...
- `object_key`의 `/`는 **퍼센트 인코딩하지 않는다**. 그 외 예약 문자만 인코딩한다.
- 이 엔드포인트 업로드 한도: **300 MB**

## 제약

- AWS SDK / S3 호환 API / Wrangler CLI는 사용하지 않는다.
