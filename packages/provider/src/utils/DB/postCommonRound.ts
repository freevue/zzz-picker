...
async function postCommonRound({ common }: PlayState, cost: Cost) {
  if (isNull(common.boss)) return null
  if (includes(null, common.A.pickList) || includes(null, common.B.pickList)) return null

  try {
    return await pipe(
      [
        [common.A.pickList, cost.A] as [PickList, Cost['A']],
        [common.B.pickList, cost.B] as [PickList, Cost['B']],
      ],
      toAsync,
      ...
      map(([side, [select_1, select_2, select_3]]) => ({
        ...
        select_3,
        boss_id: common.boss,
        score: common[side].result,
        elapsed_time: common[side].time,
      })),
      map(postPartyLog),
      toArray,
...
export default postCommonRound
