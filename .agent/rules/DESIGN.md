...
name: design-system
version: "2.0.0"
description: zzz-picker의 최신 퓨어 다크 테크니컬 테마, 컬러 토큰, 타이포그래피, 레이아웃 규격서 (Google Labs DESIGN.md 오픈 표준 준수)
...
이 문서는 `zzz-picker`의 메인 애플리케이션(`@zzz-picker/renew`)에 적용되는 현대적이고 미니멀한 **퓨어 다크 테크니컬 디자인 규격서**입니다.  
Google Labs의 코딩 에이전트 전용 디자인 시스템 명세 표준(`@google/design.md`)을 준수하여 작성되었습니다.
...
| :--- | :--- |
| [컴포넌트 명세 (apps/renew/components.md)](./apps/renew/components.md) | PlayGround, HostDashboard 등 구현체 가이드 |
| [최상위 룰 인덱스 (GEMINI.md)](./GEMINI.md) | zzz-picker 전체 룰 및 지식 아키텍처 SSOT |
