...
| **W-엔진 (무기)** | [engine-registration.md](./engine-registration.md) | `engine` 스펙, 전용 캐릭터 연결, **돌파 코스트 5개 행(`engineCost`)** |
...
4. **DB 원자성**: 이미지 행과 연결 마스터 행, 관련 코스트 행을 한 `BEGIN ... COMMIT` 안에서 적재합니다. 실패하면 전체 트랜잭션을 롤백하고, 재시도는 같은 R2 URL을 사용합니다. R2 객체 자체는 DB 트랜잭션으로 롤백되지 않으므로 미연결 key를 결과에 기록합니다.
